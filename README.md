...
Login Service dockerfile (named "session" in the compose file) copies the whole config directory into the config-data named volume and puts a fernet key into the volume. Thus the key is shared by the session and fox images (Login and Fox Service), in order for the tokens generated by Login Service to be validatable by the Fox Service.
